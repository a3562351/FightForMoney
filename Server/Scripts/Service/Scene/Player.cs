...
class Player {
    ...
    public void ChangeScene(int server_id, int scene_idx)
    {

    }

    public void ChangePerScene()
    {
    ...
    public bool GetReward(List<List<int>> reward_list, bool is_check)
    {
        List<List<int>> item_list = new List<List<int>>();
        List<List<int>> attr_list = new List<List<int>>();

        foreach (List<int> reward in reward_list)
        {
            int drop_type = reward[0];
            List<int> data = reward.GetRange(1, reward.Count);
            ...
            switch(drop_type){
                ...
                default:
                    {
                        Log.ErrorFormat("GetReward Not DropType:{0}", drop_type);
                    }
                    break;
            }
        }
        ...
        if (is_check)
        {
            if (!this.GetModule<ModItem>(ModuleIdx.Item).CanAddItemList(item_list))
            {
                return false;
            }
        }
    ...
    public bool CanConsume(List<List<int>> reward_list)
    {
        return true;
    }

    public bool Consume(List<List<int>> reward_list)
    {
        if (!this.CanConsume(reward_list))
        {
            return false;
        }
        return true;
    ...
    public T GetModule<T>(ModuleIdx idx) where T : ModuleBase
    {
        ...
        return this.module_map[idx] as T;
    }
    ...
    public void SendMsg(IMessage protocol)
    {
        if (this.is_online)
        {
            Server.GetInstance().GetSocket().SendToServer(protocol, this.GetId());
        }
    }
